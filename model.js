...
import { NotFoundError, ValidationError, ConstraintViolation } from '../components/errors';
...
export default class Model {
    ...
    static findById = function(id, conditions, properties) {
        ...
        return this.db.cypherAsync({ query: query })
            .then(response => {
    ...
    static updateById = function(id, node, conditions, properties) {
        ...
        return this.db.cypherAsync({ query: query, params: params })
            .then(response => {
                if(!response.length) return null;
                return new Node(Model, response[0]);
            })
